...
function setup() {
  const props = {
    ...
    tableHeader: [{ prop: 'prop', filterable: true }],
...
describe('Filter component (src/Filter)', () => {
  ...
  it('should render self and subcomponents', () => {
    const { props, enzymeWrapper } = setup();
    expect(enzymeWrapper.find(FormGroup).props().controlId).toBe(
      'select-filter-test-keyname'
    );

    expect(Object.keys(enzymeWrapper.find(FormGroup).props()).length).toBe(2);
    ...
    expect(enzymeWrapper.find(FormControl).props().value).toBe(
      'old-test-filter'
    );
    expect(enzymeWrapper.find(FormControl).props().placeholder).toBe(
      'Enter a text!'
    );
    expect(typeof enzymeWrapper.find(FormControl).props().onChange).toBe(
      'function'
    );
    ...
    expect(enzymeWrapper.find(FontAwesome).props().additionalClass).toBe(
      'fa-fw'
    );
